...
PROFILE=['видеонаблюд','cctv','тсв','скуд','контрол доступа','шлагбаум','опс','пожарн','соуэ','слаботоч','турникет','домофон']
EXCLUDE=['физическая охрана','охранники','чоп','вооруженная охрана']
LICENSE_RULES=[
 ('Лицензия МЧС', r'(обязатель.{0,120})?(налич.{0,80})?лицензи[яи].{0,80}(мчс|пожарн)|лицензи[яи].{0,120}(мчс|пожарн)'),
 ('Лицензия ФСБ', r'(обязатель.{0,120})?(налич.{0,80})?лицензи[яи].{0,80}фсб|лицензи[яи].{0,120}фсб'),
 ('Членство СРО', r'(членств|выписк).{0,60}сро.{0,100}(обязатель|требует|налич)'),
]
EXPERIENCE_RULES=[
 ('Обязательный подтверждённый опыт', r'(опыт|исполненн).{0,100}(не менее|обязатель).{0,120}(договор|контракт)'),
]
LEGAL_RULES=[
 ('Длительная оплата',r'оплат[аы].{0,80}(60|90|120)\s*(календарн|рабоч)',18),
 ('Дополнительные работы без оплаты',r'дополнительн.{0,80}(безвозмезд|без увеличения|за счет исполнителя)',25),
 ('Одностороннее изменение объёма',r'заказчик.{0,80}(изменить|увеличить|уменьшить).{0,80}объем',18),
...
COMP_RULES=[
 ('Марка без эквивалента',r'(hikvision|dahua|trassir|bolid|perco|doorhan)(?![^\n]{0,80}эквивалент)',20),
...
class AnalysisEngine:
    ...
    def analyze(self, tender_id: int, estimate_total: float = 0, cost_total: float = 0, estimate: dict | None = None) -> dict:
        ...
        license_stop = bool(license_findings and not self.company.licenses and not self.company.sro_memberships)
        experience_stop = bool(experience_findings and self.company.confirmed_experience_contracts == 0)
        legal_risk = min(100, sum(x['weight'] for x in legal) + (35 if license_stop else 0) + (20 if experience_stop else 0))
        ...
        score = max(0, min(100, round(profile_score*.35 + (100-legal_risk)*.2 + (100-comp_risk)*.15 + (100-technical_risk)*.1 + (100-financial_risk)*.2)))
        ...
        missing = []
        if not any(d.kind == 'Проект договора' for d in docs): missing.append('Проект договора')
        if not any(d.kind == 'Техническое задание' for d in docs): missing.append('Техническое задание')
        if experience_stop: missing.append('Подтверждённый опыт исполнения договоров')
        report = {
            'tender': {'title': t.title, 'number': t.number, 'nmck': t.nmck, 'customer': t.customer},
            'score': score, 'recommendation': rec,
            'stop_factors': [x for x in [
            ...
            'legal_risks': legal, 'competition_risks': competition,
            'equipment': equipment, 'missing': missing,
            'metrics': {
                'profile_score': profile_score, 'legal_risk': legal_risk,
                'competition_risk': comp_risk, 'technical_risk': technical_risk,
                'financial_risk': financial_risk, 'estimate_total': estimate_total,
    ...
    def _find(self, text, rules):
        ...
        for name, pattern, weight in rules:
            ...
            if m:
                ...
                out.append({'name': name, 'quote': frag[:500], 'weight': weight,
    ...
    def _find_requirements(self, text, rules):
        ...
        for name, pattern in rules:
            m = re.search(pattern, text, re.I | re.S)
            if m:
                out.append({'name': name, 'quote': text[max(0,m.start()-100):m.end()+180].replace('\n',' ')[:500], 'mandatory': True})
        return out
    ...
    def _equipment(self, text):
        ...
        for name, pattern in patterns.items():
            m = re.search(pattern, text, re.I | re.S)
            if m: out.append({'name': name, 'quantity': int(m.group(1)), 'unit': 'шт.'})
        return out
