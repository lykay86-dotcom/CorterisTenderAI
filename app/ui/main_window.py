...
class MainWindow(QMainWindow):
    def __init__(
        ...
        *,
        ...
        self.brands = json.loads(
            (Path(__file__).resolve().parents[2] / "data" / "brands_ru.json").read_text(
                encoding="utf-8"
            )
        )
        self.price_repo = PriceOfferRepository(
            Path(__file__).resolve().parents[2] / "data" / "price_offers.json"
        )
    ...
    def _v14_tools_tab(self):
        ...
        self.equipment_catalog_v14 = EquipmentCatalog(
            Path(__file__).resolve().parents[2] / "data" / "equipment_catalog.json"
        )
    ...
    def load_demo(self):
        data = json.loads(
            (Path(__file__).resolve().parents[2] / "data" / "demo_tenders.json").read_text(
                encoding="utf-8"
            )
        )
        for x in data:
    ...
    def add_brand(self):
        ...
        if cat and name and name not in self.brands[cat]:
            ...
            (Path(__file__).resolve().parents[2] / "data" / "brands_ru.json").write_text(
