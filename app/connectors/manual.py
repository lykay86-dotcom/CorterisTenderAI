        ...
        try:
            ...
            if protocol == 'API':
                ...
                with urlopen(req, timeout=15) as resp:
                    return {'ok': True, 'status': getattr(resp, 'status', 200), 'content_type': resp.headers.get('Content-Type','')}
            if protocol == 'RSS':
            ...
            if protocol == 'FTP':
                ...
                ftp = FTP_TLS(host, timeout=15) if connection.endpoint.startswith('ftps://') else FTP(host, timeout=15)
                ftp.login(connection.username or 'anonymous', password or '')
                if isinstance(ftp, FTP_TLS): ftp.prot_p()
