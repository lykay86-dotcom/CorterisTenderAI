...
class UTCDateTime(TypeDecorator[datetime]):
    """Persist aware UTC datetimes and restore SQLite values with explicit UTC."""
    ...
    def process_bind_param(self, value: datetime | None, dialect: Dialect) -> datetime | None:
        ...
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("database audit datetime must be timezone-aware")
...
class UUIDAuditMixin:
    ...
    def as_dict(self) -> dict[str, Any]:
        table = type(self).__table__  # type: ignore[attr-defined]
        return json_safe({column.name: getattr(self, column.name) for column in table.columns})
