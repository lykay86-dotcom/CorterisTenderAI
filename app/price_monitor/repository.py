...
class PriceOfferRepository:
    ...
    def save(self) -> None:
        ...
        self.storage.write_text(json.dumps([x.to_dict() for x in self.offers], ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    def upsert(self, offer: PriceOffer) -> None:
        for i, current in enumerate(self.offers):
            if current.key == offer.key:
                self.offers[i] = offer
                self.save(); return
        self.offers.append(offer); self.save()
    ...
    def _offer_from_row(self, raw: dict) -> PriceOffer | None:
        ...
        if isinstance(raw_chars, str) and raw_chars.strip():
            try: chars = json.loads(raw_chars)
            except json.JSONDecodeError:
                for part in raw_chars.split(";"):
                    if "=" in part:
                        k,v=part.split("=",1); chars[k.strip()]=v.strip()
        return PriceOffer(
            supplier=supplier, brand=brand, model=model,
    ...
    def import_file(self, path: Path) -> int:
        suffix = path.suffix.lower(); rows: list[dict] = []
        if suffix == ".csv":
            ...
            raw=json.loads(path.read_text(encoding="utf-8")); rows=raw if isinstance(raw,list) else raw.get("offers",[])
        elif suffix == ".xml":
            root=ET.parse(path).getroot()
            rows=[{child.tag: child.text or "" for child in node} for node in root.findall(".//offer")]
        else: raise ValueError("Поддерживаются CSV, XLSX, JSON и XML")
        count=0
        for row in rows:
            offer=self._offer_from_row(row)
            if offer and offer.unit_price >= 0:
                self.upsert(offer); count += 1
        return count

    def history(self, brand: str, model: str) -> list[PriceOffer]:
        values=[x for x in self.offers if x.brand.lower()==brand.lower() and x.model.lower()==model.lower()]
