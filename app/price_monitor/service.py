...
class PriceSearchService:
    ...
    def search(
        self, requirement: TenderRequirement, vat_percent: float = 22.0, only_compliant: bool = True
    ) -> list[MatchResult]:
        results = []
        for offer in self.repository.offers:
            ...
            if (
                ...
                and requirement.required_model.lower() not in hay
                and not requirement.allow_equivalent
            ):
                continue
            if (
                requirement.required_brand
                and requirement.required_brand.lower() not in hay
                and not requirement.allow_equivalent
            ):
                continue
            if query and not any(t in hay for t in query.split() if len(t) > 2):
                continue
            result = self.matcher.match(requirement, offer, vat_percent)
            if result.compliant or not only_compliant:
                results.append(result)
        return sorted(results, key=lambda x: x.rank_key)

    def cheapest(
        self, requirement: TenderRequirement, vat_percent: float = 22.0
    ) -> MatchResult | None:
    ...
    def selection_variants(
        ...
        if not compliant:
            return {"cheapest": None, "optimal": None, "reliable": None, "exact": None}
        ...
        exact = next(
            (
                ...
                and x.offer.model.lower() == requirement.required_model.lower()
            ),
            None,
        )
        optimal = max(
            ...
            key=lambda x: (
                (
                    x.confidence * 2
                    + min(x.offer.warranty_months, 60)
                    + (20 if x.offer.official_supply else 0)
                    - x.offer.lead_time_days / 2
                )
                / max(x.total["total_net"], 1)
                * 1_000_000
            ),
        )
        reliable = max(
            compliant,
            key=lambda x: (
                x.offer.official_supply,
                x.offer.certificate_available,
                x.offer.warranty_months,
