...
def inspect_docx(path: Path) -> list[Check]:
    ...
    checks = [
        Check(f"{path.name}: документ не пустой", bool(text.strip() or doc.tables), "critical")
    ]
    comments = path.with_suffix(path.suffix + ".comments")
    checks.append(
        Check(f"{path.name}: отсутствуют внешние комментарии", not comments.exists(), "warning")
    )
    return checks
...
def check_package(files: list[Path], max_size_mb: float = 50) -> dict:
    ...
    for path in existing:
        if path.suffix.lower() == ".docx":
            checks.extend(inspect_docx(path))
        if path.suffix.lower() in {".xlsx", ".xlsm"}:
            checks.extend(inspect_xlsx(path))
    size = sum(x.stat().st_size for x in existing) / 1024 / 1024
    checks.append(
        Check("Размер пакета не превышает лимит", size <= max_size_mb, "critical", f"{size:.2f} МБ")
    )
    critical_failed = any(not x.ok and x.severity == "critical" for x in checks)
    return {
        "ready": not critical_failed,
        "status": "Заявка готова к подаче" if not critical_failed else "Заявка не готова",
