...
def check_application(document_paths: list[Path], required_names: list[str] | None = None) -> dict:
    ...
    for p in document_paths:
        ...
        checks.append(
            {
                "name": f"Допустимый формат: {p.name}",
                "ok": p.suffix.lower() in {".pdf", ".docx", ".xlsx", ".xls", ".zip"},
                "severity": "warning",
            }
        )
    ...
    return {
        "ready": ready,
        "status": "Заявка готова к подаче" if ready else "Заявка не готова",
        "checks": checks,
    }
