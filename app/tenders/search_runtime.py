...
class TenderSearchRuntime:
    ...
    engine: TenderSearchEngine | None = None
    search_service: CorterisTenderSearchService | None = None
    runner: TenderSearchProfileRunner | None = None
...
def create_tender_search_runtime(
    ...
    *,
    ...
    """Build the production tender graph without activating legacy search owners.

    ``max_workers`` and ``timeout_seconds`` remain accepted during the RM-140
    rollback window, but Collector owns production search execution.
    """

    del max_workers, timeout_seconds
