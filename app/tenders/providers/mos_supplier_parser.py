...
class MosSupplierApiParser:
    ...
    def parse_search(self, payload: object) -> MosSupplierParsedSearch:
        ...
        if records and not items:
            raise MosSupplierApiParseError(
                "Ответ API содержит записи, но ни одна не распознана: " + "; ".join(errors[:5])
            )
        if errors:
    ...
    def parse_card(self, payload: object) -> UnifiedTender:
        ...
        if not isinstance(record, Mapping):
            raise MosSupplierApiParseError("Карточка котировочной сессии не является JSON-объектом")
    ...
    def parse_documents(
        ...
        for index, item in enumerate(candidates, start=1):
            ...
            if not url and document_id and document_id != f"document-{index}":
                url = self.config.file_download_url_template.format(id=document_id)
            if not url:
                continue
...
def _extract_records(
    ...
    if not isinstance(payload, Mapping):
        raise MosSupplierApiParseError("Ответ поиска API не является JSON-объектом или массивом")
    ...
    return (), total or 0, ("Ответ API не содержит распознанного списка котировочных сессий.",)
...
def _parse_datetime(value: object) -> datetime | None:
    ...
    if rendered.startswith("/Date("):
        ...
        try:
            ...
            return None
    try:
        return _ensure_timezone(datetime.fromisoformat(rendered.replace("Z", "+00:00")))
    except ValueError:
        pass
    for fmt in (
        "%d.%m.%Y %H:%M:%S",
