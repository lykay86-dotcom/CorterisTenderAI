...
Feature implementation RM-130 готова к PR/Windows CI в ветке
`feat/rm-130-saved-search-profiles`: audit-first и expected-red gates соблюдены, локально focused
`82 passed`, neighbor `64 passed`, full pytest `1656 passed`; полный workflow-equivalent contour
успешен. Используются один existing `TenderSearchProfileRepository` и один `search_profiles.json`.

Не переводить RM-130 в `DONE` и не активировать RM-131 до merge feature PR, успешного exact merge-SHA
Windows Quality Gate на Python 3.12/3.13 и отдельного merged docs-only closeout.
