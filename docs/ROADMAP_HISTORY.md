...
## 2026-07-13 — RM-110 подготовлен к ревью
- Draft PR: #19 (`feat(rm-110): stabilize tender intelligence`).
...
- Полный регресс, включая crash-reporting: 701 passed.
- Ruff по изменённому RM-110 scope проходит; `ruff check .` сохраняет 768
  существующих legacy-ошибок вне этапа. До решения по baseline RM-110 остаётся
  `IN PROGRESS`, PR не объединяется и RM-111 не активируется.
