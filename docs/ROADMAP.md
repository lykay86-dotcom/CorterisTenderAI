...
- [ ] RM-126.1 — Аудит и укрепление текущего провайдера ЕИС (активный технический подэтап).

## RM-126.1 — implementation acceptance до merge
...
- подэтап остаётся `IN PROGRESS` до feature PR merge, успешного post-merge Windows Quality Gate и
  отдельного docs-only closeout; RM-127 остаётся `PLANNED`.
...
- Общий аудит RM-126 принят, однако после получения позднего технического дополнения владелец проекта
  явно переоткрыл RM-126 для подэтапа RM-126.1. RM-127 возвращён в `PLANNED` до merge реализации
  RM-126.1, успешного post-merge Windows Quality Gate и нового docs-only closeout.
