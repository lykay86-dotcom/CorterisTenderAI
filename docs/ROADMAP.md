...
- RM-126 соответствует Definition of Done и переведён в `DONE`; RM-127 назначен единственным
  `IN PROGRESS`. Application-код RM-127 начинается только после merge этого closeout и зелёного
  финального Windows Quality Gate.

- [ ] RM-127 — Новая структура вкладок (активный).
