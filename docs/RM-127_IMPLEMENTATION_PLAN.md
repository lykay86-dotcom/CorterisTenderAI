...
- [x] Production создаёт один `QMainWindow` — `ModernMainWindow`.
- [x] `TenderWorkspacePage` — `QWidget` и единственный owner текущих tabs.
- [x] Production не импортирует/создаёт `LegacyMainWindow`, не вызывает `takeCentralWidget()`.
- [x] Exact ordered top-level/nested keys, labels и stable `objectName` проверены.
- [x] Dashboard/business workflow открывают найденный row; missing ID возвращает neutral result.
- [x] Topbar использует только named compatibility API без network/search engine/Collector call.
- [x] Existing QAction identity/objectName/shortcut/enabled state сохранены; binding idempotent.
- [x] Existing menu/toolbar и C11 full-analysis routes сохранены.
- [x] Legacy wrapper строит ровно одну page и не используется bootstrap.
- [x] Close smoke не обращается к legacy wrapper.
- [x] DB/schema/versioned artifacts/dependencies не изменены.
- [x] Focused, full pytest и workflow-equivalent gate зелёные.
