...
def _context(tmp_path):
    repository = BusinessMetricsRepository(
        tmp_path / "private-user" / "business_workflow.json"
    )
    ...
    journal = SystemHealthJournal(
        tmp_path / "private-user" / "journal.json"
    )
    journal.record(
...
def test_bundle_contains_required_diagnostics_but_not_database(
    ...
    repository, _, catalog, auto, journal, snapshot = _context(
        tmp_path
    )
    ...
    with ZipFile(result.path) as archive:
        ...
        summary = json.loads(
            archive.read("database_summary.json")
        )
...
def test_bundle_redacts_paths_email_and_secrets_from_logs(
    ...
    with ZipFile(result.path) as archive:
        text = "\n".join(
            ...
            or name == "system_health_journal.txt"
        )
