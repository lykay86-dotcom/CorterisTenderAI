...
def test_analysis(tmp_path: Path):
    ...
    t = repo.create(title="Монтаж системы видеонаблюдения", nmck=1000000)
    repo.add_document(
        t.id,
        name="ТЗ.txt",
        path="x",
        kind="Техническое задание",
        text="Поставка 10 шт. камер Hikvision. Требуется авторизационное письмо производителя.",
        page_count=1,
    )
    repo.add_document(
        t.id,
        ...
        text="Дополнительные работы выполняются без увеличения цены.",
        page_count=1,
    )
