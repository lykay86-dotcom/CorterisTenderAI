...
def test_user_settings_roundtrip(tmp_path: Path):
    ...
    prefs=UserPreferences(profit_percent=35,licenses=['Лицензия МЧС'],platforms=[PlatformConnection('ЕИС','RSS','https://example.test/rss')])
    store.save(prefs); loaded=store.load()
...
def test_template_import(tmp_path: Path):
    store=UserSettingsStore(tmp_path/'settings.json')
    template_dir=tmp_path/'templates'; store.save(UserPreferences(template_dir=str(template_dir)))
    source=tmp_path/'source.docx'; source.write_bytes(b'test')
