...
def test_profit_modes():
    ...
    markup=EstimateCalculator().calculate(items,vat_percent=0,risk_percent=0,profit_percent=30,profit_mode=ProfitMode.MARKUP)
    margin=EstimateCalculator().calculate(items,vat_percent=0,risk_percent=0,profit_percent=30,profit_mode=ProfitMode.REVENUE_MARGIN)
...
def test_missing_license_is_stop_factor(tmp_path:Path):
    ...
    repo=TenderRepository(); t=repo.create(title='Монтаж пожарной сигнализации',nmck=1000000)
    repo.add_document(t.id,name='ТЗ.txt',path='x',kind='Техническое задание',text='Обязательно наличие действующей лицензии МЧС на монтаж пожарной сигнализации.',page_count=1)
